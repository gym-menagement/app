...
- [O] FCM 토큰 서버 전송 API 연동
- [O] 이용권 구매 시 만료 알림 자동 스케줄링 (로컬)
- [O] 3일 미출석 시 운동 독려 알림 자동 스케줄링 (로컬)
- [O] 백엔드 스케줄러 연동 (원격 푸시 알림)
  - [O] Firebase Admin SDK 설정
  - [O] FCM 푸시 알림 전송 서비스
  - [O] 이용권 만료 알림 스케줄러 (매일 오전 9시)
  - [O] 3일 미출석 운동 독려 스케줄러 (매일 오전 10시)
  - [O] 테스트용 푸시 알림 API
...
- [O] `fl_chart` 패키지 사용
- [O] `lib/screens/statistics_screen.dart`
- [O] `lib/providers/statistics_provider.dart`
- [O] 데이터 집계 로직
- [O] 프로필 화면에 통계 메뉴 추가
- [O] 탭 기반 UI (개요, 운동 분석, 개인 기록)
...
- [O] `lib/screens/settings_screen.dart`
- [O] `lib/providers/settings_provider.dart`
- [O] 프로필 화면에 설정 메뉴 추가
- [ ] `lib/screens/edit_profile_screen.dart` (준비 중)
- [ ] `lib/screens/change_password_screen.dart` (준비 중)
